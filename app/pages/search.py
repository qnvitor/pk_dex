...
try:
    ...
    try:
        ...
        st.warning(f"Erro ao carregar lista rápida: {e}")
        pokemon_list = []
        quick_selection = None
    
    # Verifica se há Pokémon selecionado da página anterior
    selected_pokemon = st.session_state.get('selected_pokemon')
    
    # Determina qual Pokémon buscar
    ...
    elif selected_pokemon:
        pokemon_to_search = selected_pokemon
    ...
    if api_client:
        with st.expander("📊 Estatísticas do Cache"):
            try:
                stats = api_client.db_manager.get_cache_stats()
                st.metric("Total em cache", stats['total'])
                st.metric("Válidos", stats['valid'])
                st.metric("Expirados", stats['expired'])
                
                if st.button("Limpar Cache", type="secondary"):
                    api_client.db_manager.clear_all_cache()
